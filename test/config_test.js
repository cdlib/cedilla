...
describe('config.js testing', function() {
      ...
      fs.readFile(file, function(err, data) {
          ...
          _.forEach(matches, function(match) {
            ...
            if (hierarchy.length > 1) {
              ...
              _.forEach(_config_refs[hierarchy[0]], function(item) {
                if (item.toString() === hierarchy.slice(1, hierarchy.length).toString()) {
                  exists = true;
                }
              });

              if (!exists) {
                _config_refs[hierarchy[0]].push(hierarchy.slice(1, hierarchy.length));
              }
            }
          });
        }
        ...
        if (_.size(matches2) > 0) {
          _.forEach(matches2, function(match) {
            ...
            if (typeof _config_refs.message === 'undefined') {
              _config_refs.message = [];
            }

            var exists = false;

            _.forEach(_config_refs.message, function(item) {
              if (item.toString() === cleaned.toString()) {
                exists = true;
              }
            });

            if (!exists) {
              _config_refs.message.push(cleaned);
            }
          });
        }
  ...
  it('must have at least one root item!', function() {
    ...
    _.forEach(CONFIGS.data.objects, function(def) {
      if (def.root) {
        rootCount++;
      }
    });
    ...
    if (rootCount === 0) {
      console.log('.... One of the data types defined in config/data.yaml must be declared as the root object! "root: true"!');
    }
  ...
  // ---------------------------------------------------------------------------------------------------
  it('each reference to a config value should have a value in a config!', function() {
    ...
    _.forEach(_config_refs, function(refs, config) {
      ...
      _.forEach(refs, function(ref) {
        ...
        if (!_.contains(ref, 'default_content_service') && !_.contains(ref, 'default_content_service_port') &&
          ...
          // Warning, can only currently check up to 4 levels deep
          switch (ref.length) {
            case 1:
              passed = typeof CONFIGS[config] !== 'undefined';
              if (passed) {
                passed = typeof CONFIGS[config][ref[0]] !== 'undefined';
              }
              break;
            case 2:
              passed = typeof CONFIGS[config] !== 'undefined';

              // following need to be assignments!
              if (passed) {
                passed = typeof CONFIGS[config][ref[0]] !== 'undefined';
              }
              if (passed) {
                passed = typeof CONFIGS[config][ref[0]][ref[1]] !== 'undefined';
              }
              break;
            case 3:
              passed = typeof CONFIGS[config] !== 'undefined';
              ...
              if (passed) {
                passed = typeof CONFIGS[config][ref[0]][ref[1]][ref[2]] !== 'undefined';
              }
              break;
            case 4:
              passed = typeof CONFIGS[config] !== 'undefined';
              ...
              if (passed) {
                passed = typeof CONFIGS[config][ref[0]][ref[1]][ref[2]][ref[3]] !== 'undefined';
              }
              break;
            default:
              {
        ...
        if (!passed) {
          console.log('.... "' + ref + '" does not exist in /config/' + config + '.yaml!');
        }
        assert(passed);
  ...
  it('should have services defined in /config/services.yaml', function() {
    ...
    _.forEach(CONFIGS.services.tiers, function(services) {
  ...
  it('should have valid URL targets for each service in /config/services.yaml', function() {
    ...
    _.forEach(CONFIGS.services.tiers, function(services) {
      _.forEach(services, function(def, service) {
        ...
        if (passed) {
          passed = pattern.test(def.target);
        }
        if (!passed) {
          console.log('.... service ' + service + ' MUST specify a valid URL for "target"!');
        }
        assert(passed);
      });
    });
      ...
      _.forEach(services, function(def, service) {
        ...
        if (passed) {
          _.forEach(def.item_types_returned, function(type) {
            if (passed){
              passed = _.contains(_items, type);}
          });
        }

        if (!passed) {
          console.log('.... service ' + service + ' MUST specify a "target" and an array of "item_types_returned"!');
        }
        assert(passed);
      });
    });
  ...
  it('each service referenced in /config/rules.yaml should exist in /config/services.yaml!', function() {
    ...
    _.forEach(CONFIGS.services.tiers, function(svcs) {
  ...
  it('each item referenced in /config/rules.yaml should be defined in /config/data.yaml!', function() {
    ...
    _.forEach(CONFIGS.rules.objects, function(attributes, item) {
      ...
      if (!passed) {
        console.log('.... item ' + item + ' was referenced but not defined in /config/data.yaml!');
      }
      assert(passed);
    });
  });
  ...
  it('each item attribute referenced in /config/rules.yaml should be defined in /config/data.yaml!', function() {
    ...
    _.forEach(CONFIGS.data.objects, function(def, type) {
      ...
      _.forEach(def.attributes, function(attribute) {
        if (typeof _attributes[type] === 'undefined') {
          _attributes[type] = [];
        }
        if (!_.contains(_attributes[type], attribute)) {
          _attributes[type].push(attribute);
        }
      });
      ...
      _.forEach(def.children, function(attribute) {
        ...
        if (!_.contains(_attributes[type], attribute + 's')) {
          _attributes[type].push(attribute + 's');
        }
      });
    });
    ...
    _.forEach(CONFIGS.rules.objects, function(attributes, item) {
      _.forEach(attributes, function(values, attribute) {
        ...
        if (!passed) {
          console.log('.... item ' + item + ' references attribute ' + attribute + ' but that attribute is not defined in /config/data.yaml!');
        }
        assert(passed);
      });
    });
      ...
      _.forEach(rules, function(andRule) {
        ...
        if (andRule instanceof Array) {
          _.forEach(andRule, function(orRule) {
            ...
            if (!passed) {
              console.log('.... service ' + service + ' references attribute ' + orRule + ' but that attribute is not defined in /config/data.yaml!');
            }
            assert(passed);
          });

        } else {
          passed = _.contains(_attributes[_rootItem], andRule) || andRule === 'CONSORTIAL';

          if (!passed) {
            console.log('.... service ' + service + ' references attribute ' + andRule + ' but that attribute is not defined in /config/data.yaml!');
          }
          assert(passed);
        }
