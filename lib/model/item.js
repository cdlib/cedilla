 ...
 *
 * When an undefined attribute is passed in it gets converted to a HashMap and then placed into an
 * array called 'additional'. For example passing in an attribute named 'foo' with the value of 'bar'
 * would result in the following 'additional = [{"foo":"bar"}]'
 *
...
// -----------------------------------------------------------------------------------------------
Item.prototype.getTransactions = function(){ return this._transactions; };
// -----------------------------------------------------------------------------------------------
Item.prototype.addTransaction = function(report){ this._transactions.push(report); };
...
/*
 * Returns an object from the additional array
 * that has the specified key
 */  
Item.prototype.getAdditional = function(key){
  var additional = this._attributes['additional'];
  return _.find(additional, function(i) { return i[key] });
}

/*
 * Returns the value of an object in the additional array
 * that matches key
 */ 
Item.prototype.getAdditionalVal = function(key) {
  var found_obj = this.getAdditional(key);
  if (!found_obj) return null;
