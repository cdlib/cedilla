...
function Service(name, log) {
  ...
  if (typeof name !== 'string' || name.trim() === '') {
    throw new Error(configHelper.buildMessage(CONFIGS.message.service_no_name));
  }
    ...
    response.on('end', function() {
      ...
      try {
        ...
        if (e.message.indexOf('Unexpected token') >= 0) {
          ...
          self.log.error({object: 'service.js', service_name: self._name, badresponse: _out}, CONFIGS.message.service_bad_json);
...
Service.prototype._buildError = function(transactionId, level, message) {
  var _msg = (typeof CONFIGS.message[message] !== 'undefined') ? configHelper.buildMessage(CONFIGS.message[message], [this._name]) : message;
