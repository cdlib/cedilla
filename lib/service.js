		...
		var request = http.request(options, function(response){
			...
			response.on('data', function(chunk){
				...
				if(out.length > CONFIGS['application']['service_max_response_length']){
					...
					// TODO: Force the response on end into an error condition before the switch statement happens
					// This will also catch any errors that come back from services with a HTTP 200 instead of 500!
					out = "{\"time\":\"" + now.toJSON() + "\",\"id\":\"" + requestId + 
											"\",\"api_ver\":\"" + CONFIGS['application']['service_api_version'] +
											"\",\"error\":\"" + CONFIGS['message']['service_buffer_overflow'] + "\",\"level\":\"error\"}";
