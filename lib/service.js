...
Service.prototype.getReport = function(){
  return '{"service":"' + this._name + '",' +
          ...
          '"status":"' + this._status + '",' +
          '"response:":"' + this._response.replace('"', '\"') + '"}';
}
...
Service.prototype.call = function(item, headers){  
  var _transactionId = uuid.v4(),
      _data = serializer.itemToJsonForService(_transactionId, item, this._includeAdditionalAttributes, this._flattenJson),
      self = this;

  this._id = _transactionId;
  ...
  if(typeof self._target != 'undefined' && self._target != ''){
    ...
    var _request = _http.request(_options, function(response){
      ...
      response.on('end', function(){
        ...
        this._duration = (end - _now.getTime());
        this._status = response.statusCode;
        this._response = _out;
  
        item.addTransaction(this.getReport());
            ...
            switch(response.statusCode){
              ...
              default: // ERROR!!!
                ...
                if(typeof json['error'] != 'undefined'){
                  ...
                  LOGGER.log('error', _transactionId + ': JSON did not contain "error"!');
                  LOGGER.log('eror', json);
