	...
	if(typeof this._target != 'undefined'){
		...
		var request = http.request(options, function(response){
			...
			response.on('data', function(chunk){
				...
				if(out.length > 10000){
					response.abort();
					console.log('response is too large! aborting connection with ' + self._name);
					console.log(out);
					console.log(data);
			...
			response.on('end', function(){
				...
				try{
					switch(response.statusCode){
							...
							if(json['id'] == requestId){
								if(typeof json[item.getType() + 's'] != 'undefined'){
									...
									if(typeof rslt == 'undefined'){ 
										console.log('........ ' + self._name + ' was unable to translate the response!');
										console.log('........ ' + json);
										rslt = new Error(helper.buildMessage(CONFIGS['message']['service_unable_to_translate'], ['tester'])); 
									}
					
								}else{
									console.log('........ was expecting ' + self._name + ' to return ' + item.getType() + 's');
									console.log('........ ' + json);
									rslt = new Error(helper.buildMessage(CONFIGS['message']['service_unknown_item'], ['tester'])); 
								}
					
							}else{
								console.log('........ response received was NOT from ' + self._name + '!');
								console.log('........ got: ' + json['id'] + ', expected: ' + requestId);
						...
						default: // ERROR!!!
							...
							if(json['level'] == 'fatal'){
								...
								rslt = new Error(helper.buildMessage(CONFIGS['message']['service_server_error_fatal'], ['tester']));
								
							}else if(json['level'] == 'error'){
								// Its an error, pass it on to the client for them to interpret
								rslt = new Error(helper.buildMessage(CONFIGS['message']['service_server_error'], ['tester']));
					...
					if(e.message.indexOf('Unexpected token') >= 0){
						console.log('........ ' + self._name + ' did not receive valid JSON back from the service!');
						console.log('........ ' + out);
						rslt = new Error(helper.buildMessage(CONFIGS['message']['service_bad_json'], ['tester']));
						
					}else{
						console.log('........ ' + self._name + ' encountered an error while processing the response: ' + e.message);
						console.log('........ ' + out);
						rslt = new Error(helper.buildMessage(CONFIGS['message']['service_server_error'], ['tester']));
					}
				}
				
				console.log('...... received response from ' + self._name);
		...
		request.on('error', function(e){
			...
			if(e.message.indexOf('ECONNREFUSED') >= 0){
				...
				console.log('...... ' + self._name + ' timed out.');
				callback(new Error(helper.buildMessage(CONFIGS['message']['service_timeout'], [self._name])));
