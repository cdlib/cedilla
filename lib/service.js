...
// -----------------------------------------------------------------------------------------------
Service.prototype.getTransactionId = function(){ return this._transactionId; }
// -----------------------------------------------------------------------------------------------
...
Service.prototype.call = function(item, headers){	
	var _data = serializer.itemToJsonForService(this._transactionId, item),
			self = this;
	
	LOGGER.log('debug', self._transactionId + ' : Calling ' + self._name);
	      ...
	      if(_out.length > CONFIGS['application']['service_max_response_length']){
	        _request.abort();
	      	_aborted = true;
					...
					LOGGER.log('error', self._transactionId + ' : ' + _out);
			
					self.emit('response', self._buildError('fatal', 'service_buffer_overflow'));
	    ...
	    // ---------------------------------------------------
	      ...
	      try{
	        if(!_aborted){
	          switch(response.statusCode){
	            ...
	            case 200: // SUCCESS
	              ...
	              if(_json['id'] == self._transactionId){
	                if(typeof _json[item.getType() + 's'] != 'undefined'){
	                  ...
	                  if(typeof _rslt == 'undefined'){ 
											self.emit('response', self._buildError('error', 'service_unknown_item'));
									
									  }else{
											// successfully processed JSON response from target
											LOGGER.log('debug', self._transactionId + ' : Received response from ' + self._name);
									...
									self.emit('response', self._buildError('error', 'service_wrong_response'));
	              }
								
	              break;
      
	            case 400: // BAD REQUEST
								self.emit('response', self._buildError('error', 'service_bad_request'));
	              break;
	            ...
	            default: // ERROR!!!
								...
								LOGGER.log('error', self._transactionId + ': No valid HTTP response received from ' + self._name + ' - status: ' + response.statusCode);
								LOGGER.log('error', self._transactionId + ': ' + _out);
					
								if(typeof json['error']['level'] != 'undefined'){
									self.emit('response', self._buildError(json['error']['level'], json['error']['message']));
									
								}else{
									self.emit('response', self._buildError('fatal', 'service_server_error_fatal'));
								}
	        ...
	        if(e.message.indexOf('Unexpected token') >= 0){
						self.emit('response', self._buildError('fatal', 'service_bad_json'));
				
	          LOGGER.log('error', self._transactionId + ': ' + _out);
      
	        }else{				
						self.emit('response', self._buildError('error', 'service_server_error'));
				
	          LOGGER.log('error', self._transactionId + ': ' + e.message);
	          LOGGER.log('error', self._transactionId + ': ' + _out);
						LOGGER.log('error', self._transactionId + ': ' + e.stack);
	        }
	      }
    ...
    _request.setTimeout(self._timeout, function(){
      LOGGER.log('warn', self._transactionId + ' : Timeout while trying to connect to ' + self._name);

      _request.abort();

			self.emit('response', self._buildError('warning', 'service_timeout'));
    });
    ...
    _request.on('error', function(e){
      ...
      if(e.message.indexOf('ECONNREFUSED') >= 0){
        LOGGER.log('error', self._transactionId + ' : ' + self._name + ' does not appear to be responding.');
	
				self.emit('response', self._buildError('warning', 'service_connection_refused'));
		
      }else if(e.message.indexOf('socket hang up') >= 0){
        LOGGER.log('error', self._transactionId + ' : ' + self._name + ' timed out.');

				self.emit('response', self._buildError('warning', 'service_timeout'));

      }else{
        LOGGER.log('error', self._transactionId + ' : Error occurred while connecting to ' + self._name);
				LOGGER.log('error',  self._transactionId + ' : ' + e.message);
		...
		self.emit('response', self._buildError('fatal', 'service_no_target_defined'));
  }
	
};
	
// -----------------------------------------------------------------------------------------------
Service.prototype._buildError = function(level, message){
	var _msg = (typeof CONFIGS['message'][message] != 'undefined') ? helper.buildMessage(CONFIGS['message'][message], [this._name]) : message;
	
	LOGGER.log('error', this._transactionId + ': ' + _msg);
