...
Service.prototype.call = function(item, headers, callback){
	...
	if(typeof self._translator != 'undefined'){
		...
		map = translator.translateMap(_map);
	}
	...
	LOGGER.log('info', 'Connecting to ' + self._target + ' :: ' + _data);
	...
	if(typeof self._target != 'undefined'){
			...
			response.on('end', function(){
				...
				try{
					if(!_aborted){
						...
						rslt = new Error(helper.buildMessage(CONFIGS['message'][_out], [self._name]));
					}
		...
		LOGGER.log('debug' + 'Sending: ' + _data + ' :: ' + item.toString());
