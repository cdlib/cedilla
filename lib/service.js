...
Service.prototype.call = function(item, headers){	
	...
	console.log(self._transactionId + ' : Calling ' + self._name);
	      ...
	      if(_out.length > CONFIGS['application']['service_max_response_length']){
	        _request.abort();
	      	_aborted = true;
					...
					console.log(self._transactionId + ' : ' + _out);
...
console.log(_out);
	      ...
	      try{
	        if(!_aborted){
	          switch(response.statusCode){
	            ...
	            default: // ERROR!!!
								...
								console.log(self._transactionId + ': No valid HTTP response received from ' + self._name + ' - status: ' + response.statusCode);
								console.log(self._transactionId + ': ' + _out);
					
								if(typeof json['level'] != 'undefined'){
	        ...
	        if(e.message.indexOf('Unexpected token') >= 0){
	          ...
	          console.log(self._transactionId + ': ' + e.message);
	          console.log(self._transactionId + ': ' + _out);
						console.log(self._transactionId + ': ' + e.stack);
	        }
	      }
  
				console.log(self._transactionId + ' : Received response from ' + self._name);
			});
    ...
    _request.on('error', function(e){
      ...
      if(e.message.indexOf('ECONNREFUSED') >= 0){
        console.log(self._transactionId + ' : ' + self._name + ' does not appear to be responding.');
	
				self.emit('response', self._buildError('warning', 'service_connection_refused'));
		
      }else if(e.message.indexOf('socket hang up') >= 0){
        console.log(self._transactionId + ' : ' + self._name + ' timed out.');

				self.emit('response', self._buildError('warning', 'service_timeout'));

      }else{
        console.log(self._transactionId + ' : Error occurred while connecting to ' + self._name);
				console.log( self._transactionId + ' : ' + e.message);
...
Service.prototype._buildError = function(level, message){
	var _msg = helper.buildMessage(CONFIGS['message'][message], [this._name]);
	
	console.log(this._transactionId + ': ' + _msg);
