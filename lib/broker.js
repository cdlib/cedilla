...
var Broker = function(socket, request) {
  ...
  if(typeof CONFIGS['application']['consortial_service'] != 'undefined'){
    ...
    try{
      ...
      if(typeof request.getAffiliation() == 'undefined'){
        consortial.translateIp(request.getIp(), function(code){
          request.setAffiliation(code);
        });
        
      }else{
        consortial.translateCode(request.getAffiliation(), function(ip){
          request.setIp(ip);
        });
      }
  ...
  if(typeof socket != 'undefined' && request.hasMappedItems()){
    ...
    _.forEach(request.getMappedItems(), function(item){
      ...
      _.forEach(self._services, function(service){
        service.recordRequestInformation({"api_ver": request.getServiceApiVersion(),
