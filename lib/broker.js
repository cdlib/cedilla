...
var Broker = function(sckt, rqst, log) {
  ...
  if (request) {
    ...
    self.log.error({object: 'broker.js', request: helper.itemToMap(request), socket: socket}, CONFIGS['message']['broker_bad_request']);
...
Broker.prototype._prepareTiers = function(services, referrers){
  ...
  if(_.size(tierz) > 0){
    tiers = _.sortBy(tierz, function(item){ return item.getName() });
  }
};
