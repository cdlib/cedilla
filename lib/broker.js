...
var Broker = function(socket, item) {
  ...
  if(typeof socket != 'undefined' && item instanceof Item){
    ...
    LOGGER.log('debug', 'Incoming socket - address: ' + socket.handshake.address);
    LOGGER.log('debug', 'Incoming socket - xdomain: ' + socket.handshake.xdomain);
    LOGGER.log('debug', 'Incoming socket - url: ' + socket.handshake.url);
    LOGGER.log('debug', 'Incoming socket - headers: ' + socket.handshake.headers);
    ...
    if(item.isValid()){
      ...
      self._prepareTiers(self._services, socket.handshake.headers['referer']);
