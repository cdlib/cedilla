 ...
 * ----------------------------------------------------------------------------------------------- 
 */
var Broker = function(socket, request, log) {
  var self = this;
  ...
  if(request){
    if(typeof socket != 'undefined' && request.hasReferents()){
      ...
      if(typeof socket == 'undefined'){
        self.log.error({object: 'broker.js', request: helper.itemToMap(self._request), socket: socket}, CONFIGS['message']['broker_bad_socket']);
    ...
    self.log.error({object: 'broker.js', request: helper.itemToMap(self._request), socket: socket}, CONFIGS['message']['broker_bad_request']);
    throw new Error(CONFIGS['message']['broker_bad_request']);
  }
	
	// ----------------------------------------------------------------------------
	this.on('error', function(err){
		this.log.error({object: 'broker.js'}, err.message);
	});
