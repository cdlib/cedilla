...
var Item = function(type, assignDefaults, attributes) {
	...
	configManager.getConfig('data', function(config){
		if(typeof config['objects'] != 'undefined'){ 
			self._definition = config['objects'][type];
		}
	});
	
	if(typeof type == 'string' && typeof this._definition != 'undefined'){
		this._type = type;
		...
		configManager.getConfig('message', function(config){
			throw new Error(helper.buildMessage(config['undefined_item_type'], [type]));
		});
	}
	...
	// -----------------------------------------------------------------------------------------------
	this.getAttribute = function(key){ return this._attributes.get(key); };
	// -----------------------------------------------------------------------------------------------
