    ...
    */
    ...
    var newOURL01Specializer = function() {
      ...
      var extractFromPid = function() {
        ...
        _.find(_unused, function(val, key){
          if(key == 'pid' && val.indexOf('<naxos>') >= 0){
            item.addAttribute('naxos', /<naxos>(.*)<\/naxos>/.exec(val)[1]);
          }
        });
      };
    ...
    var handleMultipleAuthors = function(authors){
      ...
      _.forEach(authors, function(author){
        ...
        if(typeof fulls == 'string'){
          // If the value in full_name does not match the last_name so we have 2 authors
          if(fulls.indexOf(author.getAttribute('last_name')) < 0){
            // Strip off the full_name from the primary author
            author.removeAttribute('full_name');
            auths.push(author);
            
            // Add the secondary author
            auths.push(new Item('author', false, {'full_name': fulls}));
          }
          
        }else{
          _.forEach(fulls, function(full){
            // If the value in full_name does not match the last_name then we have a multiple author situation
            if(full.indexOf(author.getAttribute('last_name')) < 0){
              if(!primaryFound){
                author.removeAttribute('full_name');
                auths.push(author);
                primaryFound = true;
    ...
    var genre = item.getAttribute('genre');
