...
var Tier = function(name, log) {
    ...
    if (params.new instanceof Array) {
      ...
      if (params['new'] instanceof Item) {
        ...
        self.emit('message', {
          ...
          'item': new Item('error', true, {'message': configHelper.buildMessage(CONFIGS.message.tier_unknown_item_type, [params.service])
          })
        });
      }
    }
  ...
  this.on('error', function(err) {
    this.log.error({object: 'tier.js'}, err.message);
  });
};
...
// -----------------------------------------------------------------------------------------------
Tier.prototype.getServiceCount = function() {
  ...
  if (!self._running) {
    ...
    var heartbeat = setInterval(function() {
      ...
      if ((_now.getTime() - _start.getTime()) >= self._timeout) {
        self.log.warn({object: 'tier.js'}, 'Tier ' + self._name + ' timed out!');

        clearInterval(heartbeat);
        self._running = false;

        self.emit('error', new Item('error', false, {'level': 'warning',
          'message': configHelper.buildMessage(CONFIGS.message.tier_timeout, [self._name])}));
      ...
      if ((self._complete + _.size(self._holdingQueue)) >= _total) {
        self.log.debug({object: 'tier.js'}, 'No more items to process, sending services in holding queue to next tier.');
    ...
    _.forEach(self._queue, function(service) {
      if (service instanceof Service) {
        ...
        self.emit('error', new Item('error', false, {'level': 'fatal',
          'message': configHelper.buildMessage(CONFIGS.message.tier_not_a_service, [service.getDisplayName()])}));
      }
...
Tier.prototype._callService = function(headers, service, item) {
  ...
  if (self._hasMinimumCitation(CONFIGS.rules.minimum_item_groups[service.getName()], item, service.getRequestInformation())) {

    // Error!
    service.on('error', function(error) {
      self.log.error({object: 'tier.js'}, error.message);
    ...
    service.on('success', function(items) {
      self.log.info({object: 'tier.js'}, service.getName() + ' responded.');
      ...
      self._augmentItem(item, items, function() {
        ...
        _.forEach(self._holdingQueue, function(held) {
          if (held) {
            self.log.info({object: 'tier.js'}, 'Taking ' + held.getName() + ' out of the holding queue.');
    ...
    self.log.info({object: 'tier.js'}, 'Placing ' + service.getName() + ' into the holding queue.');
...
// -----------------------------------------------------------------------------------------------
Tier.prototype._hasMinimumCitation = function(rules, item, requestInformation) {
  var ret = true;
        ...
        if (_.isArray(andRule)) {
          ...
          _.forEach(andRule, function(orRule) {
            ...
            if (item.getAttribute(orRule) instanceof Array) {
              if (_.size(item.getAttribute(orRule)) > 0) {
                ...
                if (orRule === 'CONSORTIAL' && (typeof requestInformation.requestor_ip !== 'undefined' ||
                        typeof requestInformation.requestor_affiliation !== 'undefined')) {
                  valid = true;
                }
              }
          ...
          if (item.getAttribute(andRule) instanceof Array) {
            if (_.size(item.getAttribute(andRule)) <= 0) {
              ...
              if (andRule === 'CONSORTIAL' && typeof requestInformation.requestor_ip === 'undefined' &&
                      typeof requestInformation.requestor_affiliation === 'undefined') {
                ret = false;
              }
            }
