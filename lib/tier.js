...
configManager.getConfig('rules', function(config){ rulesConfig = config; });
configManager.getConfig('data', function(config){ itemDefinitions = config['objects']; });
*/
...
//	this._itemDefinitions = itemDefinitions;
	...
	// -----------------------------------------------------------------------------------------------
	function augmentItem(currentItem, json){
	...
	function addServiceToQueue(context, socket, item, service, callback){
		...
		if(hasMinimumCitation(CONFIGS['rules']['minimum_item_groups'][service.getName()], item)){
			...
			dispatchService(socket, item, service, function(finalItem){
	...
	// -----------------------------------------------------------------------------------------------
	function dispatchService(socket, item, service, callback){
		...
		try{
			callService(service, item, function(result){
				...
				try{
					if(typeof result != 'undefined'){
						//if(typeof itemDefinitions[result.getType()] != 'undefined'){
						if(typeof CONFIGS['data']['objects'][result.getType()] != 'undefined'){
							...
							sendItemToClient(socket, service.getDisplayName(), result);
						...
						console.log(service.getName() + ' did not return any results');
					}
			...
			console.log('.... ' + e);
			socket.emit('error', encoder.htmlEncoder(e));
			callback();
		}
	};
	
	// -----------------------------------------------------------------------------------------------
	function sendItemToClient(socket, serviceName, item){
		var now = new Date(),
				self = this,
		...
		//_.forEach(itemDefinitions[item.getType()]['children'], function(child){
			...
			if(typeof item.getAttribute(child + 's') != 'undefined'){
				_.forEach(item.getAttribute(child + 's'), function(kid){
					self.sendJsonToClient(serviceName, kid);
				});
			}
		});
	}
