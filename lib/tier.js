		...
		if(params['new'] instanceof Array){
			...
			if(params['original'] instanceof Item){
				...
				_.forEach(params['new'], function(item){
					self.emit('success', {'service': params['service'], 'item': item});
				});
			
			}else{
				// The original item is somehow undefined!
				self.emit('error', {'service': params['service'], 'error': new Item('error', true, {'message': CONFIGS['message']['tier_no_original_item']})});
			}
				
		}else{
			if(params['new'] instanceof Item){
				// Its an error so just pass it along
				self.emit('error', {'service': params['service'], 'error': params['new']});

			}else{
				// Otherwise this is an unknown item so throw an error
				self.emit('error', {'service': params['service'], 'error': new Item('error', true, {'message': helper.buildMessage(CONFIGS['message']['tier_unknown_item_type'], [params['service']])})});
			}
		}
