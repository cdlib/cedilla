...
var Dispatcher = function() {
	// -----------------------------------------------------------------------------------------------
	this.process = function(socket, configManager, availableServices, citation, callback){
		...
		if(_.size(tiers) > 0){
			...
			traverseNextTier(socket, citation, [], tiers, 0, function(){
	...
	// -----------------------------------------------------------------------------------------------
	function traverseNextTier(socket, citation, additionalServices, tiers, index, callback){
		if(tiers[index]){
			console.log('.. processing tier ' + tiers[index].getName());
			
			processTier(socket, citation, additionalServices, tiers[index], function(servicesOnHold){
				// Kick off the next tier
				return traverseNextTier(socket, citation, servicesOnHold, tiers, (index + 1), callback);
			});
	...
	// -----------------------------------------------------------------------------------------------
	function processTier(socket, citation, additionalServices, tier, callback){
		setTimeout(function(){ 
			if(!_.isEmpty(additionalServices)){
				tier.addServices(additionalServices);
			}
			tier.negotiate(socket, citation, callback); 
		});
	};
